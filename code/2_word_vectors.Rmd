...
init <- textTinyR::Doc2Vec$new(token_list = tokens_d2v, 
                              ...
                              verbose = TRUE, 
                              copy_data = TRUE # if embedding matrix is large, set to FALSE for memory efficiency
                              )  
...
rownames(doc2_minmax) <- UNGD_raw$doc_id
```
...
find_similar_doc <- function(doc_num, embedding_matrix, n) {
  similarities <- embedding_matrix[doc_num, , drop = FALSE] %>%
    sim2(embedding_matrix, y = ., method = "cosine")
  
  similarities[,1] %>% sort(decreasing = TRUE) %>% head(n)
}

find_similar_doc("BOL_1990", doc2_minmax, 5)
```
